...
- Some local environments do not have `rustc`/`cargo` on `PATH`, but `run-test-cli` and `zig build minici` need them for RustGlue tests. If RustGlue tests fail with `run spawn error: error.FileNotFound`, rerun the targeted section or full minici through `nix shell nixpkgs#rustc nixpkgs#cargo -c ...`, for example `nix shell nixpkgs#rustc nixpkgs#cargo -c zig build minici --summary failures --color off`.
